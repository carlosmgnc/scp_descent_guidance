...
class opt_problem:
  ...
  def linearize(self, t, return_value = 'a'):
    ...
    if return_value == 'A' or return_value == 'z':
      D_vq = np.array([[2*q[2]*u[2]/m - 2*q[3]*u[1]/m, 2*q[2]*u[1]/m + 2*q[3]*u[2]/m, 2*q[0]*u[2]/m + 2*q[1]*u[1]/m - 4*q[2]*u[0]/m, -2*q[0]*u[1]/m + 2*q[1]*u[2]/m - 4*q[3]*u[0]/m],
                      [-2*q[1]*u[2]/m + 2*q[3]*u[0]/m, -2*q[0]*u[2]/m - 4*q[1]*u[1]/m + 2*q[2]*u[0]/m, 2*q[1]*u[0]/m + 2*q[3]*u[2]/m, 2*q[0]*u[0]/m + 2*q[2]*u[2]/m - 4*q[3]*u[1]/m],
                      [2*q[1]*u[1]/m - 2*q[2]*u[0]/m, 2*q[0]*u[1]/m - 4*q[1]*u[2]/m + 2*q[3]*u[0]/m, -2*q[0]*u[0]/m - 4*q[2]*u[2]/m + 2*q[3]*u[1]/m, 2*q[1]*u[0]/m + 2*q[2]*u[1]/m]])
    ...
    if return_value == 'B' or return_value == 'z':
      ...
      D_u = np.vstack([-u.T*self.alpha/np.linalg.norm(u), np.zeros((3, 3)), C.T/m, np.zeros((4, 3)), self.Jbinv @ rt_cr])
  ...
  def discretize(self):
    ...
    for i in range(0, self.A.shape[0]):
      for j in range(0, nsub):
        ...
        A_temp = self.rk41(self.stm_func, sub_time, A_temp, dt_sub)
        self.stm_inv = np.linalg.inv(A_temp)
        B_temp = self.rk41(self.B_func, sub_time, B_temp, dt_sub)
        C_temp = self.rk41(self.C_func, sub_time, C_temp, dt_sub)
        E_temp = self.rk41(self.E_func, sub_time, E_temp, dt_sub)
        z_temp = self.rk41(self.z_func, sub_time, z_temp, dt_sub)
  ...
  def solve_cvx_problem(self):
    ...
    cost = sigma + w_nu * cvx.norm(nu) + w_delta * cvx.norm(delta) + w_deltasigma * cvx.norm(deltasigma, 1)
    constraints = []
    ...
    for k in range(0, self.nk-1):
      constraints += [x[:, [k+1]] == self.A[k, :,:] @ x[:, [k]] + self.B[k, :, :] @ u[:, [k]]
                      + self.C[k, :, :] @ u[:, [k+1]] + self.E[:, [k]] * sigma
                      + self.z[:, [k]] + nu[:, [k]]]
    ...
    constraints += [np.cos(theta_max) <= 1 - 2*cvx.norm(H @ x[7:11, :])]
...
for i in range(opt.nk):
  DCM = np.array([[1 - 2*(x[7+2, i]**2 + x[7+3, i]**2), 2*(x[7+1, i]*x[7+2, i] + x[7+0, i]*x[7+3, i]), 2*(x[7+1, i]*x[7+3, i] - x[7+0, i]*x[7+2, i])],
                  [2*(x[7+1, i]*x[7+2, i] - x[7+0, i]*x[7+3, i]), 1 - 2*(x[7+1, i]**2 + x[7+3, i]**2), 2*(x[7+2, i]*x[7+3, i] + x[7+0, i]*x[7+1, i])],
                  [2*(x[7+1, i]*x[7+3, i] + x[7+0, i]*x[7+2, i]), 2*(x[7+2, i]*x[7+3, i] - x[7+0, i]*x[7+1, i]), 1 - 2*(x[7+1, i]**2 + x[7+2, i]**2)]])

  rt_I[:, [i]] = DCM.T @ opt.rt

qlen = 0.05 * max_lim 
thrust_vecs = DCM.T @ u[:3, :]
q = qlen*thrust_vecs
...
ax.quiver(base_y_2, base_z_2, base_x_2, -2*rt_I[1,:], -2*rt_I[2,:], -2*rt_I[0,:],
           normalize=False, arrow_length_ratio=0.1, color='black', linewidth=1.0)
...
quiver2 = ax_anim.quiver(base_y_2[0], base_z_2[0], base_x_2[0], -2*rt_I[1,0], -2*rt_I[2,0], -2*rt_I[0,0], 
            normalize=False, arrow_length_ratio=0.1, color='black', linewidth=1)

def update(frame):
  quiver.set_segments([[[base_y[frame], base_z[frame], base_x[frame]], 
                        [x[2,frame]+rt_I[1,frame], x[3,frame] + rt_I[2,frame], x[1,frame] +rt_I[0,frame] ]]])
  
  quiver2.set_segments([[[base_y_2[frame], base_z_2[frame], base_x_2[frame]], 
                        [base_y_2[frame]-2*rt_I[1,frame], base_z_2[frame] -2*rt_I[2,frame], base_x_2[frame]-2*rt_I[0,frame]]]])
